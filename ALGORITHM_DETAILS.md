...
当前体动门控计算三类特征，其中前两类使用 PVDF+压阻双通道，小波细节能量只使用 PVDF：

1. PVDF+压阻电压快速变化率：用于捕捉短时冲击、翻身、明显姿态切换。
2. PVDF+压阻包络变化率：用于捕捉慢一些的接触状态变化、姿态变化、信号幅度突变。
3. PVDF-only 小波细节能量：用于捕捉压电通道中非呼吸频段的短时宽带扰动。
...
只对 PVDF 降采样信号做 DB5 小波分解，重构细节分量：

```text
d_P[k] = waverec(D_L, D_{L-1}, ..., D_1)
```

再计算 1 s 平滑能量：

```text
Ew_P[k] = MA_{1s}(d_P[k]^2)
W_P[k] = min(Z_+(Ew_P[k]), wavelet_z_cap)
```
...
- PVDF 对振动和高频冲击有天然响应，适合使用小波细节能量。
- 压阻通道响应更慢，高频成分更可能是电路噪声，所以不计算压阻小波能量。
- 稳定心冲击或背景噪声如果长期存在，经过 MAD 标准化后不会持续给很高分。
- 小波 z-score 默认截断到 `wavelet_z_cap = 5.0`，防止安静睡眠段 MAD 极小时，轻微的正常高频波动被统计放大后单独触发体动。
...
```text
V_fused[k] = w * Z_+(vRate_P[k]) + (1 - w) * Z_+(vRate_R[k])
E_fused[k] = w * Z_+(eRate_P[k]) + (1 - w) * Z_+(eRate_R[k])

S_raw[k] = 0.4 * V_fused[k]
         + 0.4 * E_fused[k]
         + 0.2 * min(Z_+(Ew_P[k]), wavelet_z_cap)
```
...
- `eRate_c` 表示包络变化率。
- `Ew_P` 表示 PVDF 小波细节能量。
- `w = pvdf_weight`，默认 0.5。
- `wavelet_z_cap` 默认 5.0；在默认 `wavelet_motion_weight = 0.2` 下，小波项最多贡献 1.0 分，不能单独把正常呼吸片段推成体动。

解释：

- `vRate_c` 对突然冲击和翻身更敏感。
- `eRate_c` 对呼吸幅度突然变强/变弱、姿态或接触状态变化更敏感。
- `Ew_P` 对短时宽带扰动敏感，但默认只占 0.2 且有 z-score 上限，避免把稳定 BCG、正常呼吸细节或高频噪声误当成主要判据。
- 三者融合，避免只依赖单一特征。
...
```text
PVDF-only low-frequency features: w = 1.0
PR-only low-frequency features:   w = 0.0, wavelet_motion_weight = 0
Fusion:                           w = 0.5
No wavelet:                       wavelet_motion_weight = 0
```
...
- `mean_voltage_score`：PVDF+压阻融合后的电压变化率分数。
- `mean_envelope_score`：PVDF+压阻融合后的包络变化率分数。
- `mean_pvdf_wavelet_score`：PVDF-only 小波细节能量分数。
- `mean_pvdf_wavelet_energy_z`：PVDF 小波细节能量鲁棒分数。
...
- `20260414_003312.csv`：体动占比 42.37%，窗口通过率 32.11%。
- `20260414_010312.csv`：体动占比 42.99%，窗口通过率 33.24%。
